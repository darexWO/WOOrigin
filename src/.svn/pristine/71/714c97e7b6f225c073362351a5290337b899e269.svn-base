...
public class BasketBusinessProcessing {
    ...
    public void increaseReservation(OcpProizvod ocpProizvod, int currentOJ, BigDecimal narucenaKolicina, String sessionId, User user)
        ...
        if (uzStanjeZalihaSkladista == null || narucenaKolicina.multiply(ocpProizvod.getKolicinaPoPakovanju())
...
//ukoliko postoji proizvo sa maksimalnim dozvoljenim rabatom kassa sconto se ne dodeljuje
                    ...
                    System.out.println("rabat je "+woKlasifikacijaDAO.findRootKlasifikacijaByProizvod(woRezervacijaMaxRabat.getProizvod(), cs, OJ).getMaxRabat());
                    if (woRezervacijaMaxRabat.getRabat().equals(woKlasifikacijaDAO.findRootKlasifikacijaByProizvod(woRezervacijaMaxRabat.getProizvod(), cs, OJ).getMaxRabat())){
                       maxRabat = 1;
                    }
                }
