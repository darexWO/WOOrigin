...
(function($) {
    ...
    $.fn.createProductItemDiv = function(item) {
        ...
        var product="<div class='product_item' style='display: none;'>" +
                    ...
                    "</div>";
        if (item.primeniJsklPakovanje && item.jsklPakovanja.length > 1) {
            var pakovanja="<select id='pakovanje-" + item.proizvod + "' style='border:solid 1px #666666;width:80px;'>";
            $.each(item.jsklPakovanja, function(i, pak) {
                //console.log("ima vise pakovanja pro=" + item.proizvod + " pak=" + pak);
                pakovanja+="<option value='" + pak + "'>" + pak + " " + item.jedinicaMere.skracenaOznaka.toLowerCase() + "</option> "
            });
            pakovanja+="</select>"
        ...
        else product+= "<div class='kol_pak ellipsis'>Kolièina po pakovanju: " + item.kolicinaPoPakovanju + " " + item.jedinicaMere.skracenaOznaka.toLowerCase() + "</div>";
        product+=	"<div class='kolicina'>Poruèiti: : <input type='text' id='orderedQuantity-" + item.proizvod + "' value=''/>" +  item.jedinicaMereAltRef.skracenaOznaka.toLowerCase() + "</div>" +
					"<div class='zalihe ellipsis' id='raspolozivo-" + item.proizvod + "'>Zalihe: " + item.kolUAltJM + " " + item.jedinicaMereAltRef.skracenaOznaka + " / " + item.raspolozivo + " " + item.jedinicaMere.skracenaOznaka + "</div>" +
                    "<div class='potvrda'>" +
				    	"<div style='float: left;'><input type='submit' id='dialog-" + item.proizvod + "' class='btn zoom opener'  value='UVEÆAJ' style='background-color:#b2b2b2;'/></div>" +
...
$(document).ready(function() {
    ...
    $(".add_to_basket").live('click', function() {
        try {
            ...
            if ($.trim(orderedQuantity) == '' || parseFloat(orderedQuantity) <= 0) {
                ...
                $.getJSON("addToBasket", {
                        ...
                        if (result && !result.errorMsg) {
                            $('#raspolozivo-' + productId).text('Zalihe: ' + result.stanjeZalihaAlt + ' ' + result.jmAlt + ' / ' + result.stanjeZaliha + ' ' + result.jm);
	...
	/*$("#karticaHidden").datepicker({
		...
		dayNamesMin: ['Ne', 'Po', 'Ut', 'Sr', 'Èe', 'Pe', 'Su'],
    ...
    $("#formFakture a").live('click', function(){
        if ($("#userRight").val() == "true") {
            ...
            $(".error-box").text("Nemate pravo na pregled predraèuna!");
            $("#dialog-message").dialog("open");
        }
	});
